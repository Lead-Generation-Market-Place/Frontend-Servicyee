...
import { ChevronDown, LandPlot, MapPinPlus, ArrowDownWideNarrow } from 'lucide-react';
import Link from 'next/link';
import { motion, AnimatePresence } from 'framer-motion';

interface ProfessionalFiltersProps {
  selectedMile: string | null;
  setSelectedMile: (mile: string | null) => void;
    ...
    <div className={`flex-1 w-full p-4 rounded-lg shadow-md transition-colors duration-300 dark:bg-gray-800 dark:border-gray-700 bg-white border-gray-200 border`}>
      ...
      >
        <div className="flex justify-between items-center">
          ...
          <ChevronDown className={`w-5 h-5 transition-transform text-gray-500 dark:text-gray-200 ${expandedFilter === 'distance' ? 'rotate-180' : ''}`} />
        </div>
        <AnimatePresence>
            ...
            >
              <div className="space-y-2 mt-2 pl-2 text-gray-500 dark:text-gray-200 text-xs">
                {['134 miles', '1400 miles', '987 miles'].map((mile) => (
                  <div 
                    key={mile} 
                    className={`flex items-center p-2 rounded-md cursor-pointer transition-colors ${selectedMile === mile 
                  ...
                  >
                    <input 
                      ...
                      readOnly
                    />
                    <label htmlFor={mile} className="cursor-pointer">Within {mile}</label>
                  </div>
                ))}
              </div>
            </motion.div>
      ...
      >
        <div className="flex justify-between items-center">
          ...
          <ChevronDown className={`w-5 h-5 transition-transform text-gray-500 dark:text-gray-200 ${expandedFilter === 'location' ? 'rotate-180' : ''}`} />
        </div>
        <AnimatePresence>
          {expandedFilter === 'location' && (
            ...
            >
              <div className="space-y-2 mt-2 pl-2 text-gray-500 dark:text-gray-200 text-xs">
                {['Los Angeles', 'Chicago', 'New York', 'Houston'].map((location) => (
                  <div 
                    key={location} 
                    className={`flex items-center p-2 rounded-md cursor-pointer transition-colors ${selectedLocations.includes(location)
                      ? 'dark:bg-blue-900 bg-blue-100'
                      : 'dark:hover:bg-gray-700 hover:bg-gray-100'}`}
                    onClick={(e) => {
                      e.stopPropagation();
                ...
                <Link href="#" className="text-sm text-blue-500 hover:underline block mt-2">See more</Link>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
