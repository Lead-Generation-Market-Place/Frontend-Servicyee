...
const AppSidebar: React.FC<AppSidebarProps> = ({ isServiceProvider }) => {
  ...
  // Determine if a path is active (for active link styling)
  const isActive = useCallback((path: string) => pathname === path || pathname.startsWith(`${path}/`), [pathname]);

  // Automatically open the submenu if its path is active
  useEffect(() => {
          ...
          <li key={nav.name}>
            {nav.subItems ? (
              <button
                ...
                className={`menu-item group ${hasSubmenuOpen ? "menu-item-active" : "menu-item-inactive"} ${!isExpanded ? "lg:justify-center" : "lg:justify-start"}`}
              >
                {nav.icon && <span className="mr-3 text-gray-500">{nav.icon}</span>}
                {(isExpanded || isMobileOpen) && <span>{nav.name}</span>}
                {(isExpanded || isMobileOpen) && (
                  <span className={`ml-auto transition-transform duration-200 ${hasSubmenuOpen ? "rotate-180" : ""}`}>▼</span>
                )}
              </button>
            ) : (
              nav.path && (
                    ...
                    <li key={sub.name}>
                      ...
                      >
                        ...
                        <span className="flex items-center gap-1 ml-auto">
                          {sub.new && (
                            <span className={`menu-dropdown-badge ${isActive(sub.path) ? "menu-dropdown-badge-active" : "menu-dropdown-badge-inactive"}`}>new</span>
                          )}
                          {sub.pro && (
                            <span className={`menu-dropdown-badge ${isActive(sub.path) ? "menu-dropdown-badge-active" : "menu-dropdown-badge-inactive"}`}>pro</span>
                          )}
                        </span>
                      </Link>
                    </li>
  ...
  return (
    <aside
      className={`fixed mt-16 lg:mt-0 top-0 left-0 z-50 h-screen px-5 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 text-gray-900 transition-all duration-300 ease-in-out ${isExpanded || isMobileOpen ? "w-[290px]" : "w-[90px]"} ${isMobileOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}
    >
