      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          <h2 className="text-xl font-bold text-gray-800 dark:text-white">
            Popular <span className="text-sky-600 dark:text-sky-400">Categories</span>
          </h2>
          <motion.div
            whileHover={{ x: 3 }}
        ...
        <div className="relative">
          ...
          >
            {subcategories.map(({ id, name, Icon, count, color, iconColor, hoverColor }) => (
              ...
              >
                <Link
                  href={`/home-services/sub-categories/${encodeURIComponent(name.toLowerCase().replace(/\s+/g, '-'))}?id=${id}`}
                  className="group block h-full"
                  aria-label={`Browse ${name} services`}
                >
                  <div className="h-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-5 rounded-xl flex flex-col items-center text-center hover:border-sky-300 dark:hover:border-sky-500 transition-all duration-300 shadow-sm hover:shadow-md">
                    {/* Icon with gradient background */}
                    <div className={`mb-3 p-3 rounded-xl ${color} ${hoverColor} transition-all duration-500 group-hover:scale-110`}>
                      <Icon className={`w-6 h-6 ${iconColor} group-hover:text-white transition-colors`} />
                    </div>
